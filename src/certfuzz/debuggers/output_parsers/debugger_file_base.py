...
def detect_format(debugger_output_file):
    ...
    # if you got here it's because you couldn't figure out what kind of file you're dealing with
    raise UnknownDebuggerError('Unrecognized debugger for %s' % debugger_output_file)
...
class DebuggerFile(object):
    ...
    def _hashable_backtrace_string(self, level):
        ...
        if self.keep_uniq_faddr:
            try:
                self.hashable_backtrace_string = self.hashable_backtrace_string + ' ' + self.faddr
            except:
    ...
    def _read_file(self):
        ...
        try:
            with open(self.file, 'r') as f:
                ...
                self.lines = [l.strip() for l in self.debugger_output.splitlines()]
        except IOError, e:
    ...
    def _process_backtrace(self):
        ...
        if self.is_corrupt_stack:
            ...
            logger.debug("GDB detected corrupt stack. Removing backtrace line: %s", removed_bt_line)
        else:
        ...
        if len(self.module_map):
            for module in self.module_map:
                ...
                if module['start'] < frame_address < module['end']:
                    logger.debug('Found address %x in module: %s' % (frame_address, module['objfile']))
                    return True
    ...
    def _remove_unmapped_frames(self):
        for i in xrange(len(self.backtrace) - 1, -1, -1):
            ...
            if frame_address is not None:
                mapped_frame = self._is_mapped_frame(frame_address)
                if not mapped_frame:
                    logger.debug('Removing unmapped backtrace frame address: %s' % bt)
...
def _detect_and_generate(debugger_file):
    ...
    try:
        ...
        logger.warning("No class defined for type %s (%s)", bt_type, debugger_file)
        return None
...
if __name__ == '__main__':
    ...
    parser.add_option('', '--print-hashable-backtrace', dest='print_backtrace', action='store_true', default=False, help='Prints the hashable backtrace string on a second line')
    parser.add_option('', '--pattern', dest='pattern', help='File glob pattern (optional)')
